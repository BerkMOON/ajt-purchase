...
| id | order_no | from_status | to_status | operator_id | operator_name | remark | ctime |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | 10251121495060001 | 0 | 3 | 100 | 张三 | 采购员提交采购单（自动审核） | 2024-11-20 10:00:00 |
| 2 | 10251121495060001 | 3 | 3 | 0 | 系统 | 系统自动向 5 个供应商发送询价 | 2024-11-20 10:00:01 |
| 3 | 10251121495060001 | 3 | 4 | 0 | 系统 | 询价截止，3 个供应商已报价 | 2024-11-25 23:59:59 |
| 4 | 10251121495060001 | 4 | 7 | 100 | 张三 | 选择供应商 A，提交订单 | 2024-11-26 09:00:00 |
| 5 | 10251121495060001 | 7 | 8 | 100 | 张三 | 确认到货，实际到货日期：2024-11-30 | 2024-11-30 14:00:00 |
...
| status | int | NOT NULL | 1 | 状态（1: 待报价, 2: 已选中, 3: 已到货） |
...
- `status`：明细项的状态
  - 1：待报价（默认值，新建采购单明细时自动设置）
  - 2：已选中（采购员选择供应商后，系统自动更新）
  - 3：已到货（采购员手动确认商品到货后更新）

**报价单关联：**

- `quote_no`：关联 `tb_supplier_quote_item` 表的报价单号
- 当采购员选择供应商报价后，系统会将对应的 `quote_no` 写入此字段
- 通过 `quote_no` 可以关联查询到供应商报价的详细信息（价格、交货期等）

**供应商与交货信息：**

- `supplier_id`：在采购员确认选中供应商后写入，便于统计供应商命中率；
- `delivery_date`：采购员确认到货时填写实际交货日期，对账及履约监控使用。

**设计说明：**

- 表结构已简化，移除了冗余字段（如 `product_name`、`brand` 等）
- 商品详细信息通过 `sku_id` 关联商品表获取
- 供应商报价信息通过 `quote_no` 关联 `tb_supplier_quote_item` 表获取
...
- 一个采购单可以包含多个状态流转记录
- 一个采购单可以包含多个明细条目
- 状态流转表使用 `order_id`（指向主表自增 ID）与 `order_no` 双写校验
- 明细表通过业务主键 `order_no` 与主表关联
- 删除采购单时，需在同一事务内删除对应的明细行（明细表不再维护 `is_deleted` 字段）
...
LEFT JOIN tb_purchase_order_item poi ON po.order_no = poi.order_no
...
LEFT JOIN tb_purchase_order_item poi ON poi.order_no = po.order_no
...
- 采购单号必须全局唯一
- 明细表的 `order_no` 必须能在主表中找到
- 状态流转表的 `order_id` 必须在主表中存在
- 明细表的 `quote_no` 必须关联 `tb_supplier_quote_item`
- 删除采购单时，必须在同一事务中删除所有明细行（明细表无 `is_deleted` 字段）
- 状态流转记录不允许删除（保留完整历史）
- 明细行的 `quantity` 必须大于 0，`delivery_date` 仅允许在状态为 3（已到货）时写入
